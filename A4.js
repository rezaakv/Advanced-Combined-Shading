...
// Setup renderer
var text2 = document.createElement('div9');
...
var shadowMap = new THREE.WebGLRenderTarget( window.innerWidth, window.innerHeight, { minFilter: THREE.LinearFilter, magFilter: THREE.NearestFilter });
...
var negx = new THREE.TextureLoader().load('images/negx.png');
negx.minFilter = THREE.LinearFilter;
negx.anisotropy = renderer.capabilities.getMaxAnisotropy();

var negy = new THREE.TextureLoader().load('images/negy.png');
negy.minFilter = THREE.LinearFilter;
negy.anisotropy = renderer.capabilities.getMaxAnisotropy();

var negz = new THREE.TextureLoader().load('images/negz.png');
negz.minFilter = THREE.LinearFilter;
negz.anisotropy = renderer.capabilities.getMaxAnisotropy();

var posx = new THREE.TextureLoader().load('images/posx.png');
posx.minFilter = THREE.LinearFilter;
posx.anisotropy = renderer.capabilities.getMaxAnisotropy();

var posy = new THREE.TextureLoader().load('images/posy.png');
posy.minFilter = THREE.LinearFilter;
posy.anisotropy = renderer.capabilities.getMaxAnisotropy();

var posz = new THREE.TextureLoader().load('images/posz.png');
posz.minFilter = THREE.LinearFilter;
posz.anisotropy = renderer.capabilities.getMaxAnisotropy();
...
var leg_angle = { type: 'f', value: 0.0 }
...
var terrainGeometry = new THREE.PlaneBufferGeometry(12, 12);
...
var sphereGeometry = new THREE.SphereGeometry(1, 32, 32);
var sphere2 = new THREE.Mesh(sphereGeometry, envmapMaterial);
sphere2.position.set(0, 4, -20);
finalScene.add(sphere2);
...
function update() {
  ...
  var time = Date.now();
    ...
    if((Math.abs(sphere.position.x - armadilloPosition.value.x) < 2) && Math.abs(sphere.position.z - armadilloPosition.value.z) < 2) {
    sphere.position.x = -6*Math.sin(0.001*time);
    sphere.position.z = -6*Math.cos(0.001*time);
    count++;
    text2.innerHTML = "Hi there!\n  Good Job on 314.\n Score: " + count;
  }

    sphere2.position.x = 9*Math.sin(0.001*time);
    sphere2.position.z = 9*Math.cos(0.001*time);
